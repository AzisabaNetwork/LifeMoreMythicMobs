...
public class AndCondition
   ...
   public AndCondition(String line, MythicLineConfig config) {
      super(line);
      ...
      if (rawLeft != null) {
         try {
            l = getPlugin().getSkillManager().getCondition(rawLeft);
            this.logf("init: left='%s', ACTION=%s, actionVar=%s", rawLeft, this.getACTION(), this.safeActionVar());
         } catch (Throwable t) {
      ...
      if (rawRight != null) {
         try {
            r = getPlugin().getSkillManager().getCondition(rawRight);
            this.logf("init: right='%s', ACTION=%s, actionVar=%s", rawRight, this.getACTION(), this.safeActionVar());
         } catch (Throwable t) {
   ...
   public AndCondition(MythicLineConfig config) {
   ...
   private boolean and(boolean leftResult, Supplier<Boolean> rightSupplier, String where) {
      if (!leftResult) {
         this.logf("%s: left=%s, right=SKIPPED -> AND=false", where, false);
         return false;
      } else {
         boolean rightResult = rightSupplier.get();
         boolean out = rightResult;
         this.logf("%s: left=%s, right=%s -> AND=%s", where, true, rightResult, out);
         return out;
      }
   }
