...
public class ModifyAttributeMechanic extends SkillMechanic implements ITargetedEntitySkill {
   ...
   private String getAttributeDisplayName(Attribute attr) {
      if (attr == Attribute.ARMOR) {
         return "防具";
      } else if (attr == Attribute.ARMOR_TOUGHNESS) {
         return "防具強度";
      } else if (attr == Attribute.ATTACK_DAMAGE) {
         return "攻撃力";
      } else if (attr == Attribute.ATTACK_SPEED) {
         return "攻撃速度";
      } else if (attr == Attribute.KNOCKBACK_RESISTANCE) {
         return "ノックバック耐性";
      } else if (attr == Attribute.LUCK) {
         return "幸運";
      } else if (attr == Attribute.MAX_HEALTH) {
         return "最大体力";
      } else {
         return attr == Attribute.MOVEMENT_SPEED ? "移動速度" : attr.getKey().getKey().replace("generic_", "").replace("_", "").toLowerCase();
      }
   }
