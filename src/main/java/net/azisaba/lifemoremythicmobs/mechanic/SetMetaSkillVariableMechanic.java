...
public class SetMetaSkillVariableMechanic extends SkillMechanic implements INoTargetSkill, ITargetedEntitySkill, ITargetedLocationSkill {
   ...
   public SkillResult apply(SkillMetadata data, AbstractEntity preferTarget) {
   ...
   private static SetMetaSkillVariableMechanic.VarScopePair resolveScopeAndVar(String param0, String param1) {
      MetaSkillVariableStore.Scope configuredScope = param1 == null || param1.trim().isEmpty()
            ? null
            : MetaSkillVariableStore.parseScope(param1);
      String variable = param0 == null ? "" : param0.trim();
      MetaSkillVariableStore.Scope inlineScope = null;
      int separator = variable.indexOf('.');
      if (separator > 0) {
         String prefix = variable.substring(0, separator).trim();
         switch (prefix.toUpperCase(Locale.ROOT)) {
            case "TARGET", "SKILL", "WORLD", "CASTER", "GLOBAL" -> {
               inlineScope = MetaSkillVariableStore.parseScope(prefix);
               variable = variable.substring(separator + 1).trim();
            }
            default -> {
            }
         }
      }
      MetaSkillVariableStore.Scope scope = configuredScope != null
            ? configuredScope
            : inlineScope != null ? inlineScope : MetaSkillVariableStore.Scope.CASTER;
      return new SetMetaSkillVariableMechanic.VarScopePair(scope, variable);
