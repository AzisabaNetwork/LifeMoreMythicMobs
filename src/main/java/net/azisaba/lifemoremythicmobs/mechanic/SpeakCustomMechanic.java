...
public class SpeakCustomMechanic extends Aura implements INoTargetSkill, ITargetedEntitySkill {
   ...
   public SpeakCustomMechanic(SkillExecutor executor, MythicLineConfig config) { super(executor, null, config.getLine(), config);
      ...
      this.auraName = Optional.of(PlaceholderString.of("#speakcustom"));
   ...
   public class SpeakCustomTracker extends AuraTracker {
      ...
      public void auraStart() {
         ...
         PacketTextDisplay.PacketTextDisplayBuilder options = PacketTextDisplay.create()
               .lineWidth(200)
               .backgroundColor(MythicBukkit.inst().getConfiguration().getHologramSpeechBackgroundColor())
               .billboard(MythicBukkit.inst().getConfiguration().getHologramSpeechBillboarding())
               .yOffset(SpeakCustomMechanic.this.offset)
               .cullingDistance(SpeakCustomMechanic.this.radius);
         this.bubble = MythicBukkit.inst().getHologramManager()
               .createSpeechBubble(caster, () -> this.players, options);
