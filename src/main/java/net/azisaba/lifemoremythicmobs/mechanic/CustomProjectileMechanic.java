...
public class CustomProjectileMechanic extends ProjectileMechanic {
   ...
   private final double heightFromSurface;

   public CustomProjectileMechanic(SkillExecutor executor, MythicLineConfig config) { super(executor, null, config.getLine(), config);
      ...
      this.hugSurface = config.getBoolean(new String[]{"hugsurface", "hug", "hs"}, false);
      this.heightFromSurface = config.getDouble(new String[]{"heightfromsurface", "surfaceheight", "hfs"}, 0.5D);
   }
   ...
   private AbstractVector getPerpendicularAxis(AbstractVector param1, String param2) {
      Vector direction = new Vector(param1.getX(), param1.getY(), param1.getZ());
      Vector perpendicular;
      if ("y".equalsIgnoreCase(param2)) {
         Vector horizontal = direction.clone().setY(0).normalize();
         perpendicular = horizontal.crossProduct(direction).normalize();
      } else {
         Vector horizontalAxis = direction.clone().crossProduct(new Vector(0, 1, 0));
         perpendicular = direction.clone().crossProduct(horizontalAxis).normalize();
      }
      return new AbstractVector(perpendicular.getX(), perpendicular.getY(), perpendicular.getZ());
      /* Historical bytecode retained below for migration verification.
   ...
   private Vector getZigzagRotationAxis(Vector param1, String param2) {
      Vector direction = param1.clone().normalize();
      if ("x".equalsIgnoreCase(param2) || "z".equalsIgnoreCase(param2)) {
         Vector horizontal = direction.clone().setY(0);
         if (horizontal.lengthSquared() < 1.0E-6) {
            return new Vector(1, 0, 0);
         }
         return new Vector(0, 1, 0).crossProduct(horizontal).normalize();
      }
      return param1.clone().crossProduct(new Vector(0, 1, 0)).normalize();
      /* Historical bytecode retained below for migration verification.
      // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
      // java.lang.ClassCastException: class org.jetbrains.java.decompiler.modules.decompiler.exps.AssignmentExprent cannot be cast to class org.jetbrains.java.decompiler.modules.decompiler.exps.VarExprent (org.jetbrains.java.decompiler.modules.decompiler.exps.AssignmentExprent and org.jetbrains.java.decompiler.modules.decompiler.exps.VarExprent are in unnamed module of loader 'app')
      //   at org.jetbrains.java.decompiler.modules.decompiler.SwitchHelper.findSyntheticDupVar(SwitchHelper.java:430)
   ...
   public class CustomProjectileTracker extends ProjectileTracker {
      ...
      public void projectileStart() {
         if (CustomProjectileMechanic.this.fromOrigin) {
            this.startLocation = this.data.getOrigin().clone();
         } else {
            this.startLocation = this.data.getCaster().getEntity().getLocation().clone();
         }

         double startY = CustomProjectileMechanic.this.startYOffset.get(this.context);
         float startForward = CustomProjectileMechanic.this.startForwardOffset.get(this.context);
         double startSide = CustomProjectileMechanic.this.startSideOffset.get(this.context);
         if (startY != 0.0D) {
            this.startLocation.setY(this.startLocation.getY() + startY);
         }

         if (startForward != 0.0F) {
            this.startLocation = MythicUtil.move(this.startLocation, startForward, 0.0, 0.0);
         }

         if (startSide != 0.0D) {
            this.startLocation = MythicUtil.move(this.startLocation, 0.0, 0.0, startSide);
         }
         ...
         this.currentVelocity = new AbstractVector((float)direction.getX(), (float)direction.getY(), (float)direction.getZ())
            .multiply(this.velocityMagnitude);
      ...
      public void projectileMove() {
         ...
         if (this.mechanic.pathType.equals("sine")) {
            ...
            double moveStep = this.velocityMagnitude;
            ...
            Vector move = forward.clone().multiply(this.velocityMagnitude);
         ...
         this.currentLocation
            .add(
               this.baseDirection.clone().multiply(this.velocityMagnitude).add(offset)
            );
         if (this.mechanic.hugSurface) {
            ...
            float gravity = this.mechanic.projectileGravity.get(this.context);
            if (gravity != 0.0F) {
               this.currentVelocity.setY(this.currentVelocity.getY() - gravity / this.mechanic.ticksPerSecond);
            }
         }
         ...
         if (this.mechanic.stopOnHitGround && !BlockUtil.isPathable(BukkitAdapter.adapt(this.currentLocation).getBlock(), this)) {
            ...
            if (this.inRange != null && !this.inRange.isEmpty()) {
               ...
               for (AbstractEntity e : this.inRange) {
                  if (!e.isDead() && this.getHitbox() != null && this.getHitbox().overlaps(e.getBukkitEntity().getBoundingBox()) && !this.immune.containsKey(e.getUniqueId())) {
                     this.targets.add(e);
                     this.immune.put(e.getUniqueId(), System.currentTimeMillis());
                     break;
                  }
               }
            }
            ...
            if (this.mechanic.onTickSkill.isPresent() && ((Skill)this.mechanic.onTickSkill.get()).isUsable(this.data)) {
               ...
               AbstractLocation location = this.mechanic.bulletType.isPresent() && this.mechanic.bulletType.get() == ProjectileBulletType.ARROW ? this.previousLocation.clone() : this.currentLocation.clone();
