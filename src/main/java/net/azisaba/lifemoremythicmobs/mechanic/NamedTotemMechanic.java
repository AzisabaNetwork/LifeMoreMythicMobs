...
public class NamedTotemMechanic extends Projectile implements ITargetedEntitySkill, ITargetedLocationSkill {
   ...
   public NamedTotemMechanic(SkillExecutor executor, MythicLineConfig config) { super(executor, null, config.getLine(), config);
      ...
      public void projectileTick() {
         if (this.inRange != null && this.inRange.size() > 0) {
            for (AbstractEntity e : this.inRange) {
               if (!e.isDead() && this.getHitbox() != null && this.getHitbox().overlaps(e.getBukkitEntity().getBoundingBox())) {
                  this.targets.add(e);
         ...
         if (NamedTotemMechanic.this.onTickSkill.isPresent() && ((Skill)NamedTotemMechanic.this.onTickSkill.get()).isUsable(this.data)) {
            ...
            AbstractLocation location = NamedTotemMechanic.this.bulletType.isPresent() && NamedTotemMechanic.this.bulletType.get() == ProjectileBulletType.ARROW ? this.previousLocation.clone() : this.currentLocation.clone();
