...
public class EllipseFollowMechanic extends SkillMechanic implements ITargetedEntitySkill {
   ...
   private SkillResult doCastAtEntity(final SkillMetadata data, AbstractEntity target) {
   ...
   private static EllipseFollowMechanic.ParticleSpec buildSpecForEllipse(
      ...
      switch (type) {
         case ENTITY_EFFECT: {
            ...
            return new EllipseFollowMechanic.ParticleSpec(type, Math.max(1, amount), 0.0, 0.0, 0.0, speed, c);
         }
         case DUST: {
            Color c = parseColor(colorStr);
         ...
         case ITEM:
            ...
            Material itemMaterial = Material.matchMaterial(itemStr.toUpperCase(Locale.ROOT));
            ...
            return new EllipseFollowMechanic.ParticleSpec(type, Math.max(1, amount), 0.0, 0.0, 0.0, speed, new ItemStack(itemMaterial));
         case BLOCK:
         case FALLING_DUST:
            if (blockStr == null) {
            ...
            Material blockMaterial = Material.matchMaterial(blockStr.toUpperCase(Locale.ROOT));
            ...
            return new EllipseFollowMechanic.ParticleSpec(type, Math.max(1, amount), 0.0, 0.0, 0.0, speed, blockMaterial.createBlockData());
         default:
