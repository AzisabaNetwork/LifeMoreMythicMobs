...
public class RingAroundOriginTargeter extends ILocationSelector {
   ...
   public RingAroundOriginTargeter(SkillExecutor executor, MythicLineConfig config) {
   ...
   public HashSet<AbstractLocation> getLocations(SkillMetadata data) {
      ...
      double offsetX = this.xoffset.get(data);
      double offsetY = this.yoffset.get(data);
      double offsetZ = this.zoffset.get(data);
      if (count <= 0) {
         ...
         for (int i = 0; i < count; i++) {
            ...
            Vector pos = new Vector(x + offsetX, offsetY, z + offsetZ);
            if (doRotate) {
